...
from forms import LadderGameForm, AddPlayerForm, ChallengeGameForm, JoinForm
from flask import Blueprint, make_response, redirect, render_template, request, url_for
from flask_bootstrap import Bootstrap
from functools import wraps
from math import ceil
from models import db, Player, Match, Week, Year
    ...
    for i in range(1, len(weekly_points) + 1):
        ...
        for j in range(1, i + 1):
            for k in range(len(cumulative_weekly_points_with_players[f"Wk {i}"])):
                cumulative_weekly_points_with_players[f"Wk {i}"][players_in_order[k].name] += \
                    weekly_points[f"Wk {j}"][
...
def ladder_games():
    ...
    previous_week_arrangement = [
        name for name in dict(
            sorted(
                cumulative_weekly_points_with_players[f"Wk {previous_week}"].items(),
                key=lambda x: x[1]
            )
        )
    ][::-1]
...
@main.route("/ladder-match-singles", methods=['GET', 'POST'])
@admin_only
...
@main.route("/ladder-match-doubles", methods=['GET', 'POST'])
@admin_only
...
@main.route("/generate-match", methods=['GET', 'POST'])
@admin_only
def generate_match():
...
@main.route("/challenge-match", methods=['GET', 'POST'])
@admin_only
def challenge_match():
    if request.args.get('match_id'):
        match = Match.query.get(request.args.get('match_id'))
    ...
    if request.method == 'POST':
        ...
        week = form.week.data
        form_year = form.year.data
        new_match = Match(score1=set1_score_1, score2=set1_score_2, set2_score1=set2_score_1,
                          set2_score2=set2_score_2,
...
@main.route("/delete-match/<int:match_id>")
@admin_only
